...
public class PlasmacitoidDendriticCell extends Cell {

	public PlasmacitoidDendriticCell(Grid<Cell> space) {
		super(space);
	...
	public void step() {
		...
		long nmast = StreamSupport.stream(context.getObjects(PlasmacitoidDendriticCell.class).spliterator(), false).count();
		//System.out.println(rand + " " + (double) (100-(nfat - fatCutoff))/100);
		if((double) ((nfat - fatCutoff) * 0.1) > nmast) {
			PlasmacitoidDendriticCell p = new PlasmacitoidDendriticCell(space);
			context.add(p);
